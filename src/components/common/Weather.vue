...
<template>
  ...
  >
      <div class="weather__infor--wrap" >
          <div class="welcom-message">오늘은 <strong>{{ todayWeather }} </strong>입니다.</div>
...
<script setup>
     ...
     const fetch_weather = () => {
        axios.get(`https://api.openweathermap.org/data/2.5/weather?q=Seoul,kr&APPID=13b55b2bf5bf4b64df063ddbfe1f3c5c&units=metric`)
        .then(res => {
          console.log(res.data);
          const baseUrl = 'http://openweathermap.org/img/w/' + res.data.weather[0].icon + '.png';          
          ...
          WeatherIcon.value = baseUrl;
          let ghours =new Date().getHours() < 10? "0" + new Date().getHours(): new Date().getHours();   
          let sunrise = res.data.sys.sunrise;
          let sunriseDate = new Date(sunrise*1000);
          let morningHuous = sunriseDate.getHours() < 10? "0" + sunriseDate.getHours(): sunriseDate.getHours();
          let morningMinutes = sunriseDate.getMinutes() < 10? "0" + sunriseDate.getMinutes(): sunriseDate.getMinutes(); 
          let sunriseTime = morningHuous + '시 ' + morningMinutes + '분';
          sunriseTimeIs.value = sunriseTime;
          
          let sunset = res.data.sys.sunset;
          let sunsetDate = new Date(sunset*1000);
          let sunsetHuous = sunsetDate.getHours() < 10? "0" + sunsetDate.getHours(): sunsetDate.getHours();
     ...
     const getDayOfWeek = () => { //ex) getDayOfWeek('2022-06-13')
...
.welcom-message strong {font-weight: bold;}
</style>
